			...
			<IonContent className="ion-padding">
				<IonList>
						...
						<Collapse in={exerciseOptionsOpen}>
							<IonCardContent>
								<IconButtonContainer style={{ width: "100%", height: "50px" }}>
									<IconButton
										image={walk_img}
										onClick={() => props.history.push(`/activity_submission/patient/${patientId}/type/walk`)}
										ripple
									/>
									<IconButton
										image={run_img}
										onClick={() => props.history.push(`/activity_submission/patient/${patientId}/type/run`)}
										ripple
									/>
									<IconButton
										image={cycle_img}
										onClick={() => props.history.push(`/activity_submission/patient/${patientId}/type/cycle`)}
										ripple
									/>
								</IconButtonContainer>
							</IonCardContent>
					...
					<ImageCard
						...
						onClick={() => props.history.push(`/blood_pressure_activity_submission/patient/${patientId}`)}
					/>
					<ImageCard
						title="Weight"
						image={weight_img}
						onClick={() => props.history.push(`/weight_activity_submission/patient/${patientId}`)}
					/>
				</IonList>
			</IonContent>
		</IonPage>
