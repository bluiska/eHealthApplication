...
import { IonPage, IonContent, IonCard, IonCardContent, IonCardHeader, IonCardTitle } from "@ionic/react";
...
const Patients = props => {
	...
	const Patient = data => {
		return (
			<IonCard routerDirection="forward" routerLink={`/patientoverview/doctor/${doctor}/patient/${data.patient.id}`}>
				<IonCardHeader>
				...
				{doctor && (
					<Fragment>
						...
						{dummyData
							...
							.map((patients, i1) => patients.patients.map((patient, i2) => <Patient key={i1 + i2} patient={patient} />))}
					</Fragment>
				)}
			</IonContent>
		</IonPage>
