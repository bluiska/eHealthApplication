...
const BloodPressure = props => {
	...
	const [validation, setValidation] = useState({ sysError: false, diaError: false });
	const [validationMessage, setValidationMessage] = useState({ sysError: "", diaError: "" });
		...
		<ActivitySubmissionPage
			...
			<IonItem style={{ marginTop: validation.sysError ? "-8px" : "-20px" }}>
				...
				<IonInput
					...
					onIonChange={e => {
						if (e.detail.value <= 300 && e.detail.value >= 0) {
							...
							let valMsg = {
								...validationMessage,
								sysError: e.detail.value > 300 ? "The value is too high" : "The value is too low."
							};
				...
				<Collapse in={validation.sysError}>
					<IonText style={{ color: "maroon" }}>{validationMessage.sysError}</IonText>
				</Collapse>
			</IonItem>
			<IonItem>
				<IonLabel style={styles.label} position="stacked">
				...
				<IonInput
					...
					onIonChange={e => {
						if (e.detail.value <= 300 && e.detail.value >= 0) {
							...
							let valMsg = {
								...validationMessage,
								diaError: e.detail.value > 300 ? "The value is too high" : "The value is too low."
							};
							setValidationMessage(valMsg);
						}
					}}
				...
				<Collapse in={validation.diaError}>
					<IonText style={{ color: "maroon" }}>{validationMessage.diaError}</IonText>
				</Collapse>
			</IonItem>
		</ActivitySubmissionPage>
	);
