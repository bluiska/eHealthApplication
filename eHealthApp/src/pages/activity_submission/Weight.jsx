...
const Weight = props => {
	...
	const enterWeightValue = value => {
		if (value <= 999 && value >= 0) {
			...
			let valMsg = {
				...validationMessage,
				weightError: value > 999 ? "The value is too high" : "The value is too low."
			};
	...
	return (
		<ActivitySubmissionPage
			...
			submitData={{ weight: weight, type: "weight" }}
			title="Weight entry"
					...
					<Row>
						<Col style={{ padding: "0px" }}>
							<IonItem style={{ borderRadius: "15px" }}>
								...
								<IonRadio value="kg" color="tertiary" checked="true" disabled={weight === 0 ? true : false} />
							</IonItem>
						</Col>
						<Col style={{ padding: "0px" }}>
							<IonItem style={{ borderRadius: "15px" }}>
								<IonLabel>Stones</IonLabel>
								<IonRadio value="stones" color="tertiary" disabled={weight === 0 ? true : false} />
							</IonItem>
						</Col>
						<Col style={{ padding: "0px" }}>
							<IonItem style={{ borderRadius: "15px" }}>
								<IonLabel>Pounds</IonLabel>
								<IonRadio value="pounds" color="tertiary" disabled={weight === 0 ? true : false} />
							</IonItem>
						</Col>
					</Row>
				</Container>
