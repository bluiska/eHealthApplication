...
const PatientOverview = props => {
  ...
  const RecordCard = props => {
    return props.data
      .filter(
        val => selectedFilter.includes(val.name) || selectedFilter.length === 0
      )
      .map((d, i) => (
        <IonCard key={i}>
          <IonCardContent>
  ...
  useIonViewWillEnter(async () => {
    ...
    setUserData([
      ...
      },
      {
        name: "Blood pressure",
        value: "135 mmHg",
        date: yesterdaysDate.toDateString(),
        time: "05:43"
      },
      {
        name: "Blood pressure",
  ...
  return (
    <IonPage>
      {!displayFilter && (
        <BackButtonToolbar
          title={props.match.params.name + "'s " + "Overview"}
        />
      )}
