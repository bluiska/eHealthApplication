...
import { IonCard, IonCardHeader, IonIcon, IonLabel, IonCardContent, IonList } from "@ionic/react";
...
const RecordCard = props => {
	const activityType = props.data.type;
	...
	const renderActivityValues = (activityType, data) => {
		...
		switch (activityType) {
			case ACTIVITY_TYPES.BLOOD_PRESSURE_READING:
				return (
					<div>
						...
						<IonLabel>Diastolic pressure: {data.diastolicPressure}mmHg</IonLabel>
			...
			case ACTIVITY_TYPES.WEIGHT_READING:
				...
				return (
					<div>
						<IonLabel>Weight: {parseFloat(data.weight).toFixed(2)}kg</IonLabel>
						<IonLabel>Time submitted: {date}</IonLabel>
					</div>
				);
			case ACTIVITY_TYPES.CYCLING:
				...
				return (
					<div>
						{data.caloriesBurnt && (
							<IonLabel>Calories burnt: {data.caloriesBurnt === -1 ? "N/A" : data.caloriesBurnt.toFixed(2)}</IonLabel>
						)}
						{data.distance && <IonLabel>Distance travelled: {data.distance.toFixed(2)}km</IonLabel>}
						<IonLabel>Start time: {startDate}</IonLabel>
						<IonLabel>End time: {endDate}</IonLabel>
						<IonLabel>Time submitted: {endDate}</IonLabel>
						<IonLabel>Duration: {moment.utc(moment(data.endTime).diff(moment(data.startTime))).format("HH:mm:ss")}</IonLabel>
					</div>
				);
			case ACTIVITY_TYPES.RUNNING:
				...
				return (
					<div>
						...
						{data.distance && <IonLabel>Distance travelled: {data.distance.toFixed(2)}km</IonLabel>}
						{data.steps && <IonLabel>Steps taken: {data.steps === -1 ? "N/A" : data.steps}</IonLabel>}
						<IonLabel>Start time: {startDate}</IonLabel>
						<IonLabel>End time: {endDate}</IonLabel>
						<IonLabel>Time submitted: {endDate}</IonLabel>
						<IonLabel>Duration: {moment.utc(moment(data.endTime).diff(moment(data.startTime))).format("HH:mm:ss")}</IonLabel>
		...
		<Accordion>
			<IonCard key={props.index}>
				<Accordion.Toggle as={IonCardHeader} eventKey={props.index}>
					<Row>
						<Col xs="2">
							...
							{activityType === ACTIVITY_TYPES.CYCLING && <Image src={cycle_img} style={styles.cycleImg} />}
							{activityType === ACTIVITY_TYPES.RUNNING && <Image src={run_img} style={styles.iconimg} />}
							{activityType === ACTIVITY_TYPES.WALKING && <Image src={walk_img} style={styles.iconimg} />}
						</Col>
						<Col xs="6">
							<IonLabel style={styles.title}>{renderActivityTitle(activityType)}</IonLabel>
						</Col>
						<Col xs="4">
				...
				<Accordion.Collapse eventKey={props.index}>
					<IonCardContent>{renderActivityValues(activityType, props.data)}</IonCardContent>
				</Accordion.Collapse>
			</IonCard>
		</Accordion>
	);
